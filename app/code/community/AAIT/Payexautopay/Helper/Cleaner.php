...
class AAIT_Payexautopay_Helper_Cleaner extends Mage_Core_Helper_Abstract {
    ...
    public function forceCancel($state, $clean_time) {
        ...
        $query = "UPDATE `".Mage::getSingleton('core/resource')->getTableName('sales_flat_order')."` SET state='canceled', status='canceled' WHERE state = '$state' AND created_at < '$clean_time';";
        $db->query($query);
        $query = "UPDATE `".Mage::getSingleton('core/resource')->getTableName('sales_flat_order_grid')."` SET status='canceled' WHERE status = '$state' AND created_at < '$clean_time';";
        $db->query($query); */

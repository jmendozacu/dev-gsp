...
class Payexautopay_Px_Px
{
    ...
    protected static $_rules = array(
        ...
        'PxOrderWSDL' => array(
            'AddOrderAddress2', 'AddSingleOrderLine', 'AddSingleOrderLine2', 'AuthorizeEVC', 'AuthorizeGC', 'AuthorizeInvoice',
    ...
    public function __call($px_function, $arguments)
    {
        if (empty($this->_account_number) || empty($this->_encryption_key)) {
            throw new Payexautopay_Px_Exception('Account number or Encryption key not defined. Use setEnvironment().');
        }

        $wdsl = $this->getWDSL($px_function);
        if ($wdsl === false || empty($wdsl)) {
            throw new Payexautopay_Px_Exception('Unknown PayEx Method.');
        }

        if (!isset($arguments[0]) || !is_array($arguments[0])) {
            throw new Payexautopay_Px_Exception('Invalid PayEx Method params.');
        }
        ...
        // Call PayEx Method
        $px = Payexautopay_Px_Soap::getClient($wdsl);
        try {
            ...
            if (property_exists($result, $px_function . 'Result') === false) {
                throw new Payexautopay_Px_Exception('Invalid PayEx Response.');
            }
            $result = $result->{$px_function . 'Result'};
            $result = $this->parseFields($result);
            if ($result === false) {
                throw new Payexautopay_Px_Exception('Failed to parse PayEx Response.');
            }
            return $result;
        } catch (Exception $e) {
            throw new Payexautopay_Px_Exception($e->getMessage());
        }

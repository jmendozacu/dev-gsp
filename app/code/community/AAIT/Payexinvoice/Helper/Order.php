<?php
/**
 * PayEx Invoice Helper: Order
 * Created by AAIT Team.
 */
class AAIT_Payexinvoice_Helper_Order extends Mage_Core_Helper_Abstract
{
    ...
    public function processPaymentTransaction(Mage_Sales_Model_Order $order, array $fields)
    {
        ...
        if (count($collection) > 0) {
            Mage::helper('payexinvoice/tools')->addToDebug(sprintf('Transaction %s already processed.', $fields['transactionNumber']), $order->getIncrementId());
        ...
        switch ($transaction_status) {
            case 1:
                ...
                if ($fields['pending'] === 'true') {
                    $message = Mage::helper('payexinvoice')->__('Transaction Status: %s.', $transaction_status);
            ...
            case 6:
                ...
                break;
            case 2:
                $message = Mage::helper('payexinvoice')->__('Detected an abnormal payment process (Transaction Status: %s).', $transaction_status);
                $transaction = $payment->addTransaction(Mage_Sales_Model_Order_Payment_Transaction::TYPE_PAYMENT, null, true, $message);
                $transaction->setAdditionalInformation(Mage_Sales_Model_Order_Payment_Transaction::RAW_DETAILS, $fields);
                $transaction->setMessage($message);
                ...
                break;
            case 4;
                $message = Mage::helper('payexinvoice')->__('Order automatically canceled. Transaction is canceled.');
                ...
                break;
            case 5;
                $message = Mage::helper('payexinvoice')->__('Order automatically canceled. Transaction is failed.');
                $message .= ' ' . Mage::helper('payexinvoice/tools')->getVerboseErrorMessage($fields);
                $transaction = $payment->addTransaction(Mage_Sales_Model_Order_Payment_Transaction::TYPE_PAYMENT, null, true);
                $transaction->setAdditionalInformation(Mage_Sales_Model_Order_Payment_Transaction::RAW_DETAILS, $fields);
                $transaction->setMessage($message);
                $transaction->setIsCancel(true);
                ...
                break;
            default:
                $message = Mage::helper('payexinvoice')->__('Invalid transaction status.');
        ...
        try {
            $order->save();
            Mage::helper('payexinvoice/tools')->addToDebug($message, $order->getIncrementId());
        } catch (Exception $e) {
            Mage::helper('payexinvoice/tools')->addToDebug('Error: ' . $e->getMessage(), $order->getIncrementId());
        }
    ...
    public function makeInvoice(&$order, $online = false)
    {
        ...
        if (version_compare($magento_version, '1.4.2', '>=')) {
            ...
            $invoice_id = $invoice->create($order->getIncrementId(), $order->getAllItems(), Mage::helper('payexinvoice')->__('Auto-generated from PayEx module'), false, false);
            ...
            $invoice->addComment(Mage::helper('payexinvoice')->__('Auto-generated from PayEx module'), false, false);
    ...
    public function getCalculatedOrderAmount($order, $order_amount = 0)
    {
        ...
        // add Fee
        $fee = $order->getPayexinvoicePaymentFee();
        if ($fee > 0) {
            $amount += (int)(100 * $fee);
        }
    ...
    public function addOrderLine($orderRef, $order)
    {
        ...
        foreach ($items as $item) {
            ...
            //    Mage::helper('payexinvoice/tools')->addToDebug('Warning: The product has a discount. There might be problems.', $order->getIncrementId());
            //}
            ...
            $result = Mage::helper('payexinvoice/api')->getPx()->AddSingleOrderLine2($params);
            Mage::helper('payexinvoice/tools')->debugApi($result, 'PxOrder.AddSingleOrderLine2');
            $i++;
        }
        ...
        if ($fee > 0) {
            $params = array(
                ...
                'orderRef' => $orderRef,
                'itemNumber' => $i,
                'itemDescription1' => Mage::helper('payexinvoice')->__('Payment fee'),
                'itemDescription2' => '',
                'itemDescription3' => '',
                'itemDescription4' => '',
                'itemDescription5' => '',
                'quantity' => 1,
                'amount' => (int)(100 * $fee), //must include tax
                'vatPrice' => 0,
                'vatPercent' => 0
            );
        ...
        if ((float)$order->getBaseRewardCurrencyAmount() > 0) {
            $params = array(
                ...
                'orderRef' => $orderRef,
                'itemNumber' => $i,
                'itemDescription1' => Mage::helper('payexinvoice')->__('Reward points'),
                'itemDescription2' => '',
                'itemDescription3' => '',
                'itemDescription4' => '',
                'itemDescription5' => '',
    ...
    public function addOrderAddress($orderRef, $order)
    {
        ...
        $result = Mage::helper('payexinvoice/api')->getPx()->AddOrderAddress2($params);
        Mage::helper('payexinvoice/tools')->debugApi($result, 'PxOrder.AddOrderAddress2');
        return true;
    }
}
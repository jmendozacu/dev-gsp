 ...
 */
class AAIT_Wywallet_Helper_Order extends Mage_Core_Helper_Abstract
{
    ...
    public function makeInvoice(&$order, $online = false)
    {
        ...
        if (version_compare($magento_version, '1.4.2', '>=')) {
            ...
            $invoice_id = $invoice->create($order->getIncrementId(), $order->getAllItems(), Mage::helper('wywallet')->__('Auto-generated from PayEx module'), false, false);
            ...
            $invoice->addComment(Mage::helper('wywallet')->__('Auto-generated from PayEx module'), false, false);
    ...
    public function makeCreditMemo(&$order, &$invoice, $amount, $online = false, $transactionId = null)
    {
        ...
        $creditmemo->addComment(Mage::helper('wywallet')->__('Auto-generated from PayEx module'));
    ...
    public function getOrderAmount($order)
    {
        ...
        if ($abs > 0 && $abs < 0.011) {
            Mage::helper('wywallet/tools')->addToDebug('Warning: Price rounding issue. ' . $grand_total . ' vs ' . $amount);
            return $amount;
        } else {
            return $grand_total;
        }
